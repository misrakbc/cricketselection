"""
generate_app.py
Generates index.html and predictor.html as ONE CONTINUOUS LONG PAGE with the exact order requested:
- Module 1: 📊 Coach Selection Philosophies & Red-Ball Deep Dive
- Module 2: 📋 Empirical Playing XI Team Compositions Across All Formats
- Module 3: 🏏 Live Coach Selection Predictor & Custom Cricketer Creator
"""
...
<body class="bg-transparent text-[var(--foreground)] antialiased p-3 sm:p-5 font-sans">
  <div class="max-w-6xl mx-auto bg-[var(--card)] text-[var(--foreground)] border border-[var(--border)] rounded-2xl p-4 sm:p-6 shadow-md space-y-8">
    ...
    <!-- Sticky Quick Jump Navigation Bar (Reordered) -->
    <nav class="sticky top-2 z-20 bg-[var(--background)]/90 backdrop-blur-md p-2 rounded-xl border border-[var(--border)] shadow-sm">
      <div class="flex flex-wrap items-center justify-center sm:justify-start gap-2 text-xs sm:text-sm font-bold">
        <a href="#philosophy" class="px-3.5 py-2 rounded-lg bg-indigo-600 text-white hover:bg-indigo-700 transition flex items-center gap-1.5 shadow-xs">
          <span>📊</span>
          <span>1. Coach Philosophies & Red-Ball Deep Dive</span>
        </a>
        <a href="#compositions" class="px-3.5 py-2 rounded-lg bg-[var(--secondary)] hover:bg-[var(--secondary)]/80 text-[var(--foreground)] transition flex items-center gap-1.5 border border-[var(--border)]">
          <span>📋</span>
          <span>2. Empirical Playing XI Compositions</span>
        </a>
        <a href="#predictor" class="px-3.5 py-2 rounded-lg bg-[var(--secondary)] hover:bg-[var(--secondary)]/80 text-[var(--foreground)] transition flex items-center gap-1.5 border border-[var(--border)]">
          <span>🏏</span>
          <span>3. Selection Predictor & Custom Player Builder</span>
        </a>
      </div>
    </nav>
    ...
    <!-- MODULE 1: COACH SELECTION PHILOSOPHIES & RED-BALL DEEP DIVE -->
    <!-- ========================================================================= -->
    <section id="philosophy" class="space-y-8 pt-2">
      
      <!-- Section Header -->
      <div class="space-y-2 border-b border-[var(--border)] pb-3">
    ...
    <!-- ========================================================================= -->
    <!-- MODULE 2: EMPIRICAL PLAYING XI TEAM COMPOSITIONS -->
    <!-- ========================================================================= -->
    <section id="compositions" class="space-y-6 pt-8 border-t-2 border-[var(--border)]">
      
      <div class="space-y-2 border-b border-[var(--border)] pb-3">
        <div class="flex items-center gap-2">
          <span class="px-2 py-0.5 text-xs font-extrabold rounded bg-blue-500/20 text-blue-400 uppercase tracking-wide">Module 2</span>
      ...
      <!-- Quick Jump Button to Module 3 -->
      <div class="pt-2 text-center">
        <a href="#predictor" class="px-5 py-2.5 text-xs font-bold rounded-xl bg-indigo-600 hover:bg-indigo-700 text-white transition shadow-xs inline-flex items-center gap-1.5">
          <span>↓</span>
          <span>Jump to Module 3: Selection Predictor & Custom Player Builder</span>
        </a>
      </div>
    ...
    <!-- ========================================================================= -->
    <!-- MODULE 3: LIVE COACH SELECTION PREDICTOR & CUSTOM PLAYER BUILDER -->
    <!-- ========================================================================= -->
    <section id="predictor" class="space-y-6 pt-8 border-t-2 border-[var(--border)]">
      
      <!-- Section Title -->
      <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-[var(--border)] pb-3">
        <div>
          <div class="flex items-center gap-2">
            <span class="px-2 py-0.5 text-xs font-extrabold rounded bg-indigo-500/20 text-indigo-400 uppercase tracking-wide">Module 3</span>
      ...
      <!-- Controls Row: Format & Conditions (Prominently Selectable) -->
      <div class="bg-[var(--background)] p-4 rounded-xl border-2 border-[var(--border)] shadow-sm space-y-3">
        
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
          
          <!-- Player Select (For Existing Mode) -->
          <div id="playerSelectContainer" class="space-y-1.5">
            <label class="block text-xs font-bold uppercase tracking-wider text-[var(--foreground)] flex items-center gap-1.5">
              <span>👤</span>
              <span>Select Indian Player</span>
            </label>
            <select id="playerSelect" onchange="updateExistingView()" class="w-full bg-[var(--card)] text-[var(--foreground)] border-2 border-[var(--border)] hover:border-indigo-400 rounded-xl px-3.5 py-2.5 text-xs sm:text-sm font-bold shadow-xs focus:outline-none focus:ring-2 focus:ring-indigo-500 cursor-pointer transition">
              {''.join([f'<option value="{p}">{p}</option>' for p in PLAYERS_DATABASE.keys()])}
            </select>
          </div>

          <!-- Format Selector: Ultra Visible & Tactile -->
          <div class="space-y-1.5">
            <div class="flex items-center justify-between">
              ...
              <span class="text-[10px] font-extrabold px-2 py-0.5 rounded-full bg-indigo-500/20 text-indigo-400">
                Click to Switch
              </span>
            </div>
            <div class="grid grid-cols-3 gap-1.5 bg-[var(--card)] p-1.5 rounded-xl border-2 border-[var(--border)] text-center text-xs font-bold shadow-xs">
              <button id="btnTEST" onclick="setFormat('TEST')" class="py-2 px-1 rounded-lg bg-indigo-600 text-white font-extrabold shadow-md border-2 border-indigo-400 flex items-center justify-center gap-1 cursor-pointer transition transform scale-[1.02]">
                <span>🔴</span>
                <span>TEST</span>
                <span id="checkTEST" class="text-[11px] font-black">✓</span>
              </button>
              <button id="btnODI" onclick="setFormat('ODI')" class="py-2 px-1 rounded-lg bg-[var(--background)] hover:bg-[var(--secondary)] text-[var(--foreground)] font-semibold border-2 border-[var(--border)] hover:border-indigo-400/60 shadow-xs flex items-center justify-center gap-1 cursor-pointer transition transform active:scale-95">
                <span>🔵</span>
                <span>ODI</span>
                <span id="checkODI" class="hidden text-[11px] font-black">✓</span>
              </button>
              <button id="btnT20I" onclick="setFormat('T20I')" class="py-2 px-1 rounded-lg bg-[var(--background)] hover:bg-[var(--secondary)] text-[var(--foreground)] font-semibold border-2 border-[var(--border)] hover:border-indigo-400/60 shadow-xs flex items-center justify-center gap-1 cursor-pointer transition transform active:scale-95">
                <span>🟢</span>
                <span>T20I</span>
                <span id="checkT20I" class="hidden text-[11px] font-black">✓</span>
              </button>
            </div>
          </div>

          <!-- Venue Selector: Ultra Visible & Tactile -->
          <div class="space-y-1.5">
            <div class="flex items-center justify-between">
              <label class="block text-xs font-bold uppercase tracking-wider text-[var(--foreground)] flex items-center gap-1.5">
                <span>🌍</span>
              ...
              <span class="text-[10px] font-extrabold px-2 py-0.5 rounded-full bg-emerald-500/20 text-emerald-400">
                Click to Switch
              </span>
            </div>
            <div class="grid grid-cols-2 gap-1.5 bg-[var(--card)] p-1.5 rounded-xl border-2 border-[var(--border)] text-center text-xs font-bold shadow-xs">
              <button id="btnHome" onclick="setVenue('Home')" class="py-2 px-2 rounded-lg bg-emerald-600 text-white font-extrabold shadow-md border-2 border-emerald-400 flex items-center justify-center gap-1.5 cursor-pointer transition transform scale-[1.02]">
                <span>🏟️</span>
                <span>Subcontinent</span>
                <span id="checkHome" class="text-[11px] font-black">✓</span>
              </button>
              <button id="btnSena" onclick="setVenue('SENA')" class="py-2 px-2 rounded-lg bg-[var(--background)] hover:bg-[var(--secondary)] text-[var(--foreground)] font-semibold border-2 border-[var(--border)] hover:border-emerald-400/60 shadow-xs flex items-center justify-center gap-1.5 cursor-pointer transition transform active:scale-95">
                <span>✈️</span>
                <span>SENA Overseas</span>
                <span id="checkSena" class="hidden text-[11px] font-black">✓</span>
              </button>
            </div>
          </div>
        ...
        <!-- Live Active Context Confirmation Banner -->
        <div class="pt-2.5 border-t border-[var(--border)] flex flex-wrap items-center justify-between gap-2 text-xs">
          <div class="flex items-center gap-2">
            <span class="w-2 h-2 rounded-full bg-emerald-400 animate-ping"></span>
            <span class="text-[var(--muted-foreground)] font-medium">Currently Selected Scenario:</span>
            <span id="activeContextBanner" class="font-extrabold text-[var(--foreground)] px-2.5 py-0.5 rounded-md bg-[var(--secondary)] border border-[var(--border)]">
              TEST Match • Subcontinent Pitch
            </span>
          </div>
          <span class="text-[11px] text-indigo-400 font-medium">
            💡 Click buttons above to switch format/venue in real time
          </span>
        </div>
      ...
      <div class="pt-4 text-center">
        <a href="#philosophy" class="px-5 py-2.5 text-xs font-bold rounded-xl bg-[var(--secondary)] hover:bg-[var(--secondary)]/80 text-[var(--foreground)] border border-[var(--border)] transition shadow-xs inline-flex items-center gap-1.5">
          <span>↑</span>
          <span>Back to Top: Module 1 Coach Philosophies</span>
        </a>
      </div>
  ...
  <script>
    ...
    function setFormat(fmt) {{
      ...
      ['TEST', 'ODI', 'T20I'].forEach(f => {{
        ...
        const check = document.getElementById('check' + f);
        if (f === fmt) {{
          btn.className = "py-2 px-1 rounded-lg bg-indigo-600 text-white font-extrabold shadow-md border-2 border-indigo-400 flex items-center justify-center gap-1 cursor-pointer transition transform scale-[1.02]";
          if (check) check.classList.remove('hidden');
        }} else {{
          btn.className = "py-2 px-1 rounded-lg bg-[var(--background)] hover:bg-[var(--secondary)] text-[var(--foreground)] font-semibold border-2 border-[var(--border)] hover:border-indigo-400/60 shadow-xs flex items-center justify-center gap-1 cursor-pointer transition transform active:scale-95";
    ...
    function setVenue(v) {{
      ...
      if (v === 'Home') {{
        btnHome.className = "py-2 px-2 rounded-lg bg-emerald-600 text-white font-extrabold shadow-md border-2 border-emerald-400 flex items-center justify-center gap-1.5 cursor-pointer transition transform scale-[1.02]";
        btnSena.className = "py-2 px-2 rounded-lg bg-[var(--background)] hover:bg-[var(--secondary)] text-[var(--foreground)] font-semibold border-2 border-[var(--border)] hover:border-emerald-400/60 shadow-xs flex items-center justify-center gap-1.5 cursor-pointer transition transform active:scale-95";
        if (checkHome) checkHome.classList.remove('hidden');
        if (checkSena) checkSena.classList.add('hidden');
      }} else {{
        btnSena.className = "py-2 px-2 rounded-lg bg-emerald-600 text-white font-extrabold shadow-md border-2 border-emerald-400 flex items-center justify-center gap-1.5 cursor-pointer transition transform scale-[1.02]";
        btnHome.className = "py-2 px-2 rounded-lg bg-[var(--background)] hover:bg-[var(--secondary)] text-[var(--foreground)] font-semibold border-2 border-[var(--border)] hover:border-emerald-400/60 shadow-xs flex items-center justify-center gap-1.5 cursor-pointer transition transform active:scale-95";
        if (checkSena) checkSena.classList.remove('hidden');
        if (checkHome) checkHome.classList.add('hidden');
      }}
      updateActiveBanner();
      if (currentSubMode === 'existing') updateExistingView(); else calcCustomPlayer();
    }}
    ...
    function updateActiveBanner() {{
      ...
      if (banner) {{
        const venueLabel = currentVenue === 'Home' ? 'Subcontinent Pitch (Home / Asia)' : 'SENA Overseas (AUS, ENG, SA, NZ)';
        banner.innerText = `${{currentFormat}} Match • ${{venueLabel}}`;
      }}
    }}
...
print("SUCCESS: Successfully reordered page modules and updated index.html and predictor.html!")
